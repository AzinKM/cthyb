...
class nrg_solver:
    ...
    def _calc_occupancy(self, Delta, Sigma):
    ...
    def _adjust_mu(self, Delta_in, Sigma):
        ...
        for _ in range(self.par_dic['max_mu_adjust']):
            sol = optimize.root_scalar(lambda mu : self._calc_occupancy(Delta, Sigma)-self.par_dic['occupancy_goal'], x0=self.mu, x1=self.mu-0.1) ### I'm not sure if this will work
            self.mu = sol.root
            self.sp_dic["model_parameters"]["eps1"] = -self.mu #### why don't we write self.mp_dic["eps1"] instead of self.sp_dic["model_parameters"]["eps1"]?
    ...
    def _dmft_step(self, Delta):
        Delta_in_fixed = self._fix_hyb_function(Delta)
        ...
        occupancy = self._calc_occupancy(Delta, self.S.Sigma_w)
    ...
    def solve_from_scratch(self):
        ...
        for bl in Sigma.indices:
            for w in Sigma.mesh:
                Sigma[bl][w] = self.mp_dic['U1']*self.par_dic['occupancy_goal']/2.0  # Initialize self-energy with the Hartree shift
        self.mu = self.mp_dic['U1']/2.0 # initial approximaiton for the chemical potential
        self.sp_dic["model_parameters"]["eps1"] = -self.mu
        Delta = self._self_consistency(Sigma)
        Delta = self._adjust_mu(Delta, Sigma)
        self.Gloc_prev = self.Gloc.copy()
